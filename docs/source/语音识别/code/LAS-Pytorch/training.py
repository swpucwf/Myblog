...
if __name__ == "__main__":
    ...
    '''
    training_filepath：训练数据文件路径，默认为'data_utils/training.txt'。
    testing_filepath：测试数据文件路径，默认为'data_utils/testing.txt'。
    vocabulary_path：词汇表文件路径，默认为'data_utils/vocab.txt'。
    config_file：配置文件路径，默认为'data_utils/config.yaml'。
    save_modelpath：模型保存路径，默认为'save_models/'
    '''
    # 使用parse_args()
    # 方法解析参数，并将结果存储在args变量中。
    # 调用create_vocab_dict()
    # 函数，根据词汇表路径创建字符字典。
    # 使用with open()
    # 语句打开配置文件，并使用yaml.safe_load()
    # 方法读取文件内容，将其存储在config变量中。
    # 调用main()
    # 函数，传入config参数，执行主要任务。
